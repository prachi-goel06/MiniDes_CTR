...
class MiniDes:
    ...
    def IV(self):
        self.iv = input("Please enter an alphabet for Nonce example a etc: ")
        if len(self.iv) != 1:
            print("The value of initialization vector is not valid expected q letters got less/more ")
            sys.exit()
        return self.iv
...
def Encryption():
    ...
    binary_nonce = encryption.letters_to_binary(nonce)
    ...
    for i in range(0, len(blocks)):
        binary_counter=encryption.counter_to_binary(Counter)
        ...
        for j in range(0, len(key)):
            ...
            print('Key is: ', key[j])
            L = encryption.encryp_step1_L0(counter_nonce)
            R = encryption.encryp_step1_R0(counter_nonce)
            ...
            Encrypted_Ri = encryption.XOR_Sbox_L_to_find_R(Sbox, L)
            print("new value of L is ", R)
            counter_nonce = (R + Encrypted_Ri)
        CipherBlock = (Encrypted_Ri + R)
        Encrypted_block = encryption.PlaintextBlock_XOR_EncryptedIV(blocks[i], CipherBlock)
