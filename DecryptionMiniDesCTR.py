...
class MiniDes:
    ...
    def IV(self):
        self.iv=input("Please enter 1 alphabets for nonce example 'a' etc: ")
        if len(self.iv)!=1:
            print("The value of initialization vactor is not valid expected 2 letters got less/more ")
            sys.exit()
        return self.iv

    def counter_to_binary(self, counter):
        # counter conversion to 6 bit
        counter_binary = format(counter,'06b')
        return counter_binary
...
# computing (f(R0,K1) XOR L0)
    ...
    def plaintext_conversion(self,plaintext):
        ...
        print ("the characters are after decryption in bits are: ", characters)
        return characters
...
def Decryption():
    ...
    binary_nonce= decryption.letters_to_binary(nonce)
...
# Stream of data to be decrypted
    ...
    for i in range(0, len(blocks)):
        binary_counter = decryption.counter_to_binary(Counter)
        print("Value of counter is: ", binary_counter)
        print("Block is ", i + 1, ": ", blocks[i])
        print("nonce for block: ", binary_nonce)
        counter_nonce = binary_nonce + binary_counter
        print("value of counter and nonce is", counter_nonce)
        for j in range(0, len(key)):
            # 2 round encryption for each block of plaintext
            print('Key is: ', key[j])
            L = decryption.encryp_step1_L0(counter_nonce)
            R = decryption.encryp_step1_R0(counter_nonce)
            ...
            Decrypted_Ri = decryption.XOR_Sbox_L_to_find_R(Sbox, L)
            print("new value of L is ", R)
            counter_nonce = (R + Decrypted_Ri)
        CipherBlock = (Decrypted_Ri + R)
        Decrypted_block = decryption.XOR_IV_Block(blocks[i], CipherBlock)
        print("'Decryption of block", i + 1, "given by RiLi': ", Decrypted_block, '\n\n\n')
        Counter = Counter + 1
